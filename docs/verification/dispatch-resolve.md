...
Executable cases cover the strongest-class direction declaration on a rule and on the default set, rejected undeclared low floors for both, a missing confidence clearing only where the selected set both declares the strongest class and has not raised its floor, a present nonnumeric confidence returning `error` rather than uncertainty and recording no matched rule in the calibration line, and the selected set's floor governing a quota fall-through instead of the matched rule's.
