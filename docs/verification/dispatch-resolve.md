...
Executable cases cover the per-rule strongest-class direction declaration, its rejected undeclared low floors, a missing confidence clearing only where the selected rule both declares the strongest class and has actually lowered its `confidence_floor` below the global default, and handing back where the rule holds or raises that floor, a default selection answering to the global floor with no declaration available to it, a present nonnumeric confidence returning `error` rather than uncertainty and recording no matched rule in the calibration line, and a quota fall-through taking the global floor rather than the matched rule's.
