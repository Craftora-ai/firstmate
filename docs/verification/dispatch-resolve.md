...
Executable cases cover the strongest-class direction declaration on a rule and on the default set, rejected undeclared low floors for both, missing versus malformed confidence, and the selected set's floor governing a quota fall-through instead of the matched rule's.
