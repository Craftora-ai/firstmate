...
# --- Case E (re-drive with a longer observation window): adversarial - trust only the worktree root ---
mkhome "$TMP/cx-wtroot" "$HOME/.treehouse"
printf '%s\n' "$TMP/cx-wtroot" > "$HOME_FM/config/codex-home"
run_case cxhE "trust only $HOME/.treehouse"
P=$(pane_of cxhE)
for i in $(seq 1 24); do
  txt=$(lab pane read "$P" --source recent-unwrapped --lines 300 --format text 2>/dev/null)
  echo "$txt" | grep -q -E 'Do you trust|DONE CODEX_HOME' && break
  sleep 10
done
sleep 5
lab pane read "$P" --source recent-unwrapped --lines 300 --format text > "$EVD/r3b-pane-cxhE.txt" 2>&1
echo "== pane cxhE saved after ~$((i*10))s"
grep -E -i 'subdirectory|repository root|/proj$|Do you trust|REPORT CODEX_HOME|DONE CODEX_HOME' "$EVD/r3b-pane-cxhE.txt" | sed 's/^/   /' | head
codex_env_of_pane "" cxhE | grep -E '^ *[0-9]|^(CODEX_HOME=/|FM_TASK_ID)'
