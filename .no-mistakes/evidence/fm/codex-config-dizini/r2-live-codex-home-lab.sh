...
# --- Case A: configured home ---
printf '%s\n' "$HOME/.codex-alfred" > "$HOME_FM/config/codex-home"
run_case cxhA "config/codex-home=$HOME/.codex-alfred"
# --- Case B: adversarial invalid setting refuses before any pane/meta ---
printf 'relative/codex\n' > "$HOME_FM/config/codex-home"
brief cxhB
out=$(spawn cxhB 2>&1); echo "== spawn cxhB (relative path): rc=$? ; $out" | tail -3
[ -e "$HOME_FM/state/cxhB.meta" ] && echo "cxhB meta EXISTS (bad)" || echo "cxhB: no task record written"
lab workspace list | jq -r '[.result.workspaces[]?.label]|join(",")' | sed 's/^/workspaces after refusal: /'
lab pane list | jq -r '.result.panes|length' | sed 's/^/pane count after refusal: /'
# --- Case C: control, setting absent ---
rm -f "$HOME_FM/config/codex-home"
run_case cxhC "no config/codex-home"

sleep 20
for id in cxhA cxhC; do
  echo "== running Codex process env for $id"; codex_env_of_pane "" "$id"
done
# let workers answer
for i in $(seq 1 30); do
  PA=$(pane_of cxhA); txt=$(lab pane read "$PA" --source recent-unwrapped --lines 300 --format text 2>/dev/null)
  echo "$txt" | grep -q 'DONE CODEX_HOME' && break
  sleep 10
done
for id in cxhA cxhC; do
  P=$(pane_of "$id")
  lab pane read "$P" --source recent-unwrapped --lines 300 --format text > "$EVD/r2-pane-$id.txt" 2>&1
  echo "== pane $id ($P) saved"
  grep -E 'REPORT CODEX_HOME|DONE CODEX_HOME|^ *(brave|dfs-mcp|exa|firecrawl|codex_app|n8n-mcp|node_repl) ' "$EVD/r2-pane-$id.txt" | sed 's/^/   /' | head -20
done
