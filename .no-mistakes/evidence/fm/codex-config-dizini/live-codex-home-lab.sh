...
for id in cxhA cxhC; do
  P=$(pane_of "$id")
  lab pane read "$P" --source recent-unwrapped --lines 300 --format text > "$EVD/pane-$id.txt" 2>&1
  echo "== pane $id ($P) saved"
  grep -E 'REPORT CODEX_HOME|DONE CODEX_HOME|^ *(brave|dfs-mcp|exa|firecrawl|codex_app|n8n-mcp|node_repl) ' "$EVD/pane-$id.txt" | sed 's/^/   /' | head -20
done
