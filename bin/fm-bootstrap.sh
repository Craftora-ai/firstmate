...
crew_dispatch_validate() {
  ...
  err=$(jq -r --argjson typed "$typed_active" --argjson verified_harnesses "$verified_harnesses" --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
    ...
    # Keep this boundary aligned with fm-dispatch-resolve.sh CONFIDENCE_FLOOR.
    elif $typed and any((.rules // [])[]; has("confidence_floor") and .confidence_floor < 0.6 and .strongest_reasoning != true) then "rule confidence_floor below 0.6 requires strongest_reasoning: true"
